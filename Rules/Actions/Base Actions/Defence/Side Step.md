---
tags: []
cssclass: pathfinder
---


<h1> Side Step <span style="margin-left: auto;"> Reaction </span> </h1>

[[Defence]]{.trait}

[[Skill | Skill (Acrobatics)]]{.trait}

<div style="clear:both" />

**Trigger:**  You are the target of an attack; or a source with the [[Blast]] Trait.
___

**System:**
You step out the way by taking the [[Step]] action and rolling Acrobatics against the Attack Roll. For sources with the [[Blast]] Trait you take no damage if the [[Step]] action would move you out of the blast radius.

<br>

**Success:** The attack counts as a Miss.
**Fail:** You reduce the damage taken by your Dexterity.
**Critical Fail:** You take full damage from the attack.


